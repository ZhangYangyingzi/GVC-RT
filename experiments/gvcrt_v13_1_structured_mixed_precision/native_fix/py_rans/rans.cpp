...
void RansEncoderLib::flush()
{
    ...
    // A symbol outside the tabulated CDF support emits bypass bins in addition
    // to its regular RANS symbol.  One byte per input symbol is therefore not
    // a valid upper bound and can let the backwards writer underflow `output`.
    // The packed y symbol has an 8-bit signed value, so 16 bytes per symbol plus
    // fixed state headroom is a conservative bound without changing the coder.
    const size_t output_capacity = static_cast<size_t>(total_symbol_size) * 16 + 1024;
    uint8_t* output = new uint8_t[output_capacity];
    uint8_t* ptrEnd = output + output_capacity;
    uint8_t* ptr = ptrEnd;
    assert(ptr != nullptr);
