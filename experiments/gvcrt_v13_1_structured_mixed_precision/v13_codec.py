...
def encode_binary_rle(modes):
    modes = [int(x) for x in modes]
    if not modes or any(x not in (0, 1) for x in modes): raise ValueError("binary map required")
    writer = BitWriter(); writer.put(modes[0], 1); runs = []
    start = 0
    while start < len(modes):
        mode = modes[start]; end = start + 1
        while end < len(modes) and modes[end] == mode: end += 1
        runs.append((mode, end - start)); writer.ue(end - start - 1); start = end
    data, bits = writer.bytes(); return data, bits, runs


def decode_binary_rle(data, bit_count, count):
    reader = BitReader(data, bit_count); mode = reader.get(1); out = []; runs = []
    while len(out) < count:
        length = reader.ue() + 1
        if len(out) + length > count: raise ValueError("binary RLE exceeds tile count")
        out.extend([mode] * length); runs.append((mode, length)); mode = 1 - mode
    if reader.pos != bit_count: raise ValueError("unused binary RLE bits")
    return out, runs


def encode_sparse(modes, inverse=False):
    modes = [int(x) for x in modes]
    if not modes or any(x not in (0, 1) for x in modes): raise ValueError("binary map required")
    exception = 0 if inverse else 1
    indices = [i for i, x in enumerate(modes) if x == exception]
    writer = BitWriter(); writer.ue(len(indices)); prev = -1
    for index in indices:
        writer.ue(index - prev - 1); prev = index
    data, bits = writer.bytes(); return data, bits, indices


def decode_sparse(data, bit_count, count, inverse=False):
    reader = BitReader(data, bit_count); n = reader.ue(); default = 1 if inverse else 0; exception = 1 - default
    out = [default] * count; indices = []; prev = -1
    for _ in range(n):
        index = prev + 1 + reader.ue()
        if index >= count: raise ValueError("sparse index exceeds tile count")
        out[index] = exception; indices.append(index); prev = index
    if reader.pos != bit_count: raise ValueError("unused sparse bits")
    return out, indices


def encode_mode_map_best(modes):
    rle_data, rle_bits, runs = encode_binary_rle(modes)
    sparse_data, sparse_bits, sparse_indices = encode_sparse(modes, inverse=False)
    inverse_data, inverse_bits, inverse_indices = encode_sparse(modes, inverse=True)
    candidates = [(rle_bits, len(rle_data), 1, "RASTER_RLE", rle_data, runs),
                  (sparse_bits, len(sparse_data), 3, "SPARSE_2X", sparse_data, sparse_indices),
                  (inverse_bits, len(inverse_data), 4, "INVERSE_SPARSE_1X", inverse_data, inverse_indices)]
    selected = min(candidates, key=lambda x: (x[1], x[0], x[2]))
    return {"data": selected[4], "bits": selected[0], "kind": selected[2], "syntax": selected[3],
            "detail": selected[5], "raw_bitmap_bits": len(modes), "rle_bits": rle_bits,
            "sparse_bits": sparse_bits, "inverse_sparse_bits": inverse_bits,
            "rle_bytes": len(rle_data), "sparse_bytes": len(sparse_data), "inverse_sparse_bytes": len(inverse_data)}
...
def encode_w1_by_mode(model, prepared, block_h, block_w):
    ...
    # A trajectory can legitimately have no active w1 symbols.  The native
    # RANS encoder represents that as an empty stream; do not enqueue/flush it.
    ...
    for mode in (0, 1, 2):
        ...
        # Use the repository's encoder-side symbol/index builder.  Besides
        # choosing the scale-table index it applies the codec's legal CDF
        # support handling; manually packing q/index can corrupt the native
        # RANS state when a selected q lies outside that support.
        # `select` already applies the codec's activity threshold in the
        # original scale domain.  Disable the encoder helper's second threshold
        # pass: applying it after scale/delta would silently drop coarse-mode
        # symbols that the decoder is required to read.
        saved_threshold = model.gaussian_encoder.force_zero_thres
        model.gaussian_encoder.force_zero_thres = None
        try:
            combined = model.gaussian_encoder.build_indexes_encoder(
                symbols[select], (scales / MODE_TO_DELTA[mode])[select])
        finally:
            model.gaussian_encoder.force_zero_thres = saved_threshold
        if os.environ.get("V13_DEBUG_PROBE") == "1":
            vals = symbols[select]
            print("w1_rans_mode", mode, "count", int(select.sum()), "q", int(vals.min()), int(vals.max()),
                  flush=True)
        model.entropy_coder.encode_y(combined, model.gaussian_encoder.cdf_group_index)
    model.entropy_coder.flush()
    return model.entropy_coder.get_encoded_stream()
...
def decode_w1_by_mode(model, stream, scales, modes, block_h, block_w, dtype, device):
    ...
    # RansDecInit requires a non-empty state.  An empty stream is the exact
    # representation of the zero-active-symbol case and decodes to all zeros.
    if not bool(active.any()):
        if len(stream) != 0:
            raise ValueError("non-empty w1 stream for zero active symbols")
        return result
    if len(stream) == 0:
        raise ValueError("empty w1 stream with active symbols")
...
def encode_mixed(model, prepared, qp, block_h=4, block_w=4, global_mode=None):
    ...
    if global_mode is None:
        map_info = encode_mode_map_best(prepared["modes"])
        map_stream, map_bits = map_info["data"], map_info["bits"]
        kind, gm = map_info["kind"], 255
        runs = map_info["detail"] if kind == 1 else [(0, prepared["modes"].count(0)), (1, prepared["modes"].count(1))]
    else:
        map_stream, map_bits, runs = b"", 0, [(int(global_mode), len(prepared["modes"]))]
        kind, gm = 2, int(global_mode)
        map_info = {"syntax": "GLOBAL", "raw_bitmap_bits": len(prepared["modes"]), "rle_bits": 0,
                    "sparse_bits": 0, "inverse_sparse_bits": 0, "rle_bytes": 0,
                    "sparse_bytes": 0, "inverse_sparse_bytes": 0}
    ...
    return payload, {
        ...
        "raw_map_bits": len(prepared["modes"]), "coded_map_bits": map_bits,
        "raw_bitmap_bits": map_info["raw_bitmap_bits"], "rle_bits": map_info["rle_bits"],
        "sparse_bits": map_info["sparse_bits"], "inverse_sparse_bits": map_info["inverse_sparse_bits"],
        "selected_syntax": map_info["syntax"], "selected_map_bits": map_bits,
        "aligned_map_bytes": len(map_stream),
...
def parse_mixed(payload, block_h=4, block_w=4):
    ...
    if kind == 1:
        modes, runs = decode_binary_rle(map_stream, map_bits, count)
    elif kind == 2 and global_mode in MODE_TO_DELTA and ml == 0 and map_bits == 0:
        modes, runs = [global_mode] * count, [(global_mode, count)]
    elif kind == 3:
        modes, indices = decode_sparse(map_stream, map_bits, count, inverse=False); runs = [(1, len(indices))]
    elif kind == 4:
        modes, indices = decode_sparse(map_stream, map_bits, count, inverse=True); runs = [(0, len(indices))]
    else:
    ...
    syntax = {1: "RASTER_RLE", 2: "GLOBAL", 3: "SPARSE_2X", 4: "INVERSE_SPARSE_1X"}[kind]
    return {"kind": kind, "syntax": syntax, "global_mode": global_mode, "h": h, "w": w, "modes": modes,
