...
def parse_args():
    ...
    parser.add_argument('--model_path_i', type=str,
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                             "checkpoints", "GVC_RT-I.pt"))
    parser.add_argument('--model_path_p', type=str,
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                             "checkpoints", "GVC_RT-P.pt"))
    parser.add_argument('--rate_num', type=int, default=1)
    parser.add_argument('--qp_i', type=int, nargs="+", default=[1])
    parser.add_argument('--qp_p', type=int, nargs="+", default=[1])
    ...
    parser.add_argument('--reset_interval', type=int, default=-1, required=False)
    ...
    parser.add_argument('--cuda_idx', type=str, default='0', nargs="+", help='GPU indexes to use')
...
def get_distortion(args, x_hat, y, u, v, rgb, device):
    if args['src_type'] == 'yuv420':
        ...
        if lpips_model is not None:
            rgb_t = torch.clamp(x_hat, 0.0, 1.0).to(device)
            rgb_orig = torch.from_numpy(rgb).unsqueeze(0).float() / 255.0
            rgb_orig = rgb_orig.to(device)
            with torch.no_grad():
                lpips_val = lpips_model(rgb_orig, rgb_t).item()
        else:
            lpips_val = 0.
        curr_lpips = [lpips_val]
        ...
        if dists_model is not None:
            ...
            rgb_orig = rgb_orig.to(device)
            with torch.no_grad():
                dists_val = dists_model(rgb_orig, rgb_t).item()
        else:
            dists_val = 0.
        curr_dists = [dists_val]

    return curr_psnr, curr_ssim, curr_lpips, curr_dists
...
def run_one_point_with_stream(p_frame_net, i_frame_net, args):
    ...
    target_height = max(1088, pic_height)
    target_width = max(1920, pic_width)
    padding_b = target_height - pic_height
    padding_r = target_width - pic_width

    use_two_entropy_coders = target_height * target_width > 1280 * 720
    ...
    with torch.no_grad():
        while decoded_frame_number < frame_num:
            ...
            x_hat = (torch.clamp(x_hat, -1.0, 1.0) + 1.0) / 2.0
            ...
            curr_psnr, curr_ssim, curr_lpips, curr_dists = get_distortion(args, x_hat, y, u, v, rgb, device)
            ...
            if verbose >= 2:
                ...
                print(f"frame {decoded_frame_number} decoded, {frame_time * 1000:.3f} ms, "
                      ...
                      f"LPIPS: {curr_lpips[0]:.4f} DISTS: {curr_dists[0]:.4f}")
    ...
    log_result = generate_log_json(frame_num, target_height * target_width, test_time,
                                   ...
                                   lpips_list=lpips_list, dists_list=dists_list)

    total_pixels = frame_num * target_height * target_width

    log_result['avg_psnr'] = sum([p[0] for p in psnrs]) / len(psnrs) if psnrs else 0.0
    log_result['avg_msssim'] = sum([m[0] for m in msssims]) / len(msssims) if msssims else 0.0
    log_result['avg_lpips'] = sum([l[0] for l in lpips_list]) / len(lpips_list) if lpips_list else 0.0
    log_result['avg_dists'] = sum([d[0] for d in dists_list]) / len(dists_list) if dists_list else 0.0
    log_result['avg_bpp'] = sum(bits) / total_pixels if bits else 0.0
...
def init_func(args, gpu_num):
    ...
    global i_frame_net
    i_frame_net = DMCI()
    i_frame_net = DMCI(encoder_ckpt_path=args.model_path_i)
    ...
    if not args.force_intra:
        if args.model_path_p and os.path.exists(args.model_path_p):
            print(f"Loading P-frame model from {args.model_path_p}...")
            p_checkpoint = torch.load(args.model_path_p, map_location="cpu")
            p_sd = p_checkpoint.get('student_ema',
                                    p_checkpoint.get('student', p_checkpoint.get('state_dict', p_checkpoint)))
            p_frame_net.load_state_dict(p_sd, strict=True)
        else:
            print("⚠️ Warning: Running P-frame inference with default/random weights!")
...
def main():
    ...
    for res in results:
        ...
        qp_key = (qp_i_val, qp_p_val)
        if qp_key not in qp_stats:
            qp_stats[qp_key] = []
        qp_stats[qp_key].append(res)

        print(f"--- Testing {seq} | QP_I {qp_i_val} | QP_P {qp_p_val} ---")
        print(
            f"Avg PSNR: {res['avg_psnr']:.2f} dB | Avg MS-SSIM: {res['avg_msssim']:.4f} | Avg BPP: {res['avg_bpp']:.4f}")

    print("====================================")
    print("OVERALL SUMMARY PER QP (Across all sequences)")
    print("==================================================")
    for (qpi, qpp) in sorted(qp_stats.keys()):
        stats = qp_stats[(qpi, qpp)]
        num_seqs = len(stats)

        avg_psnr = sum([s['avg_psnr'] for s in stats]) / num_seqs
        avg_msssim = sum([s['avg_msssim'] for s in stats]) / num_seqs
        avg_bpp = sum([s['avg_bpp'] for s in stats]) / num_seqs

        print(f"[QP_I{qpi}_P{qpp}] (Sequences: {num_seqs})")
        print(f" > PSNR: {avg_psnr:.2f} | MS-SSIM: {avg_msssim:.4f} | BPP: {avg_bpp:.4f}")
    print("==================================================\n")
