...
class CompressionModel(nn.Module):
    def __init__(self, z_channel, extra_qp=0):
        ...
        self.bit_estimator_z = BitEstimator(10 + extra_qp, z_channel)
    ...
    def get_z_bits(self, z, qp, bit_estimator):
    ...
    def forward_prior_4x(self, y, common_params, y_spatial_prior_reduction,
        ...
        y_res_w_0 = self.single_part_for_writing_4x(y_res_0)
        y_res_w_1 = self.single_part_for_writing_4x(y_res_1)
        y_res_w_2 = self.single_part_for_writing_4x(y_res_2)
        y_res_w_3 = self.single_part_for_writing_4x(y_res_3)
        y_res_packed = torch.cat((y_res_w_0, y_res_w_1, y_res_w_2, y_res_w_3), dim=1)

        s_hat_w_0 = self.single_part_for_writing_4x(s_hat_0)
        s_hat_w_1 = self.single_part_for_writing_4x(s_hat_1)
        s_hat_w_2 = self.single_part_for_writing_4x(s_hat_2)
        s_hat_w_3 = self.single_part_for_writing_4x(s_hat_3)
        scales_packed = torch.cat((s_hat_w_0, s_hat_w_1, s_hat_w_2, s_hat_w_3), dim=1)
        return y_res_packed, y_hat, scales_packed
