...
class Bitparm(nn.Module):
    ...
    def forward(self, x, index):
        if isinstance(index, int):
            index = torch.tensor([index], dtype=torch.long, device=self.h.device)
...
class BitEstimator(AEHelper, nn.Module):
    ...
    def get_probs(self, x, index):
        # 概率 = CDF(x + 0.5) - CDF(x - 0.5)
        # 这是计算连续加噪信号落在某个整数区间的概率的标准做法
        upper = self.get_cdf(x + 0.5, index)
        lower = self.get_cdf(x - 0.5, index)
        prob = upper - lower
        # 加上极小值防止 log(0) 导致 NaN
