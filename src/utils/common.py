...
def generate_log_json(frame_num, frame_pixel_num, test_time, frame_types, bits, psnrs, ssims,
    ...
    # LPIPS / DISTS handling
    has_lpips = lpips_list is not None and len(lpips_list) > 0 and lpips_list[0][0] != 0.
    has_dists = dists_list is not None and len(dists_list) > 0 and dists_list[0][0] != 0.
    if has_lpips:
        i_lpips = sum([lpips_list[idx][0] for idx in range(frame_num) if frame_types[idx] == 0])
        p_lpips = sum([lpips_list[idx][0] for idx in range(frame_num) if frame_types[idx] != 0])
        log_result['ave_i_frame_lpips'] = i_lpips / i_num if i_num > 0 else 0
        log_result['ave_p_frame_lpips'] = p_lpips / p_num if p_num > 0 else 0
        log_result['ave_all_frame_lpips'] = (i_lpips + p_lpips) / frame_num
        if verbose:
            log_result['frame_lpips'] = [v[0] for v in lpips_list]
    if has_dists:
        i_dists = sum([dists_list[idx][0] for idx in range(frame_num) if frame_types[idx] == 0])
        p_dists = sum([dists_list[idx][0] for idx in range(frame_num) if frame_types[idx] != 0])
        log_result['ave_i_frame_dists'] = i_dists / i_num if i_num > 0 else 0
        log_result['ave_p_frame_dists'] = p_dists / p_num if p_num > 0 else 0
        log_result['ave_all_frame_dists'] = (i_dists + p_dists) / frame_num
        if verbose:
            log_result['frame_dists'] = [v[0] for v in dists_list]
