...
class PNGReader():
    def __init__(self, src_path, width, height, start_num=None):
        self.eof = False
    ...
    def read_one_frame(self):
        ...
        if getattr(self, 'zero_based', False):
            png_path = os.path.join(self.src_path,
                                    f"{str(self.current_frame_index).zfill(self.padding)}.png"
                                    )
        else:
            png_path = os.path.join(self.src_path,
